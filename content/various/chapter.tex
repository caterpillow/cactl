...
\section{Bigger types}
	\begin{itemize}[noitemsep]
		...
		\item \texttt{long double}: 80-bit x87 on Linux (64-bit mantissa, \tilde{}19 digits). Use \texttt{sqrtl}, \texttt{powl}, \texttt{fabsl}, \texttt{\%Lf}.
		\item \texttt{\_\_float128}: 113-bit mantissa (\tilde{}34 digits), software emulated (libgcc calls, \tilde{}25$\times$ slower than \texttt{double}, \tilde{}8$\times$ slower than \texttt{long double}). Arithmetic and casts just work; \texttt{sqrtq}, \texttt{expq} and printing need \texttt{<quadmath.h>} + \texttt{-lquadmath}, so print via \texttt{(long double)} or peel digits manually.
	\end{itemize}
